...
def equationTests():
	...
	emailsNode.equation("NumberEmailsPerDay=1")
	emailsNode.levels([0,5,10,15,20,25,30,35])
	print(emailsNode)
	print()

	print("[Invalid equation doesn't change what's set]")
	print("Before:", emailsNode.equation())
	try:
		emailsNode.equation("NumberEmailsPerDay=9x-")
	except Exception as e:
		print("Exception details:\033[90m")
		traceback.print_exc()
		print("\033[0m")
	print("After:", emailsNode.equation())

	print("[New equation node, but immediately invalid]")
	eqNode = net.addNode("InvalidEqNode", nodeType=Node.EQUATION_NODE)
	print("Before: ", eqNode.equation())
	try:
		eqNode.equation("InvalidEqNode=9x-")
	except:
		print("Exception details:\033[90m")
		traceback.print_exc()
		print("\033[0m")
	print("After: ", eqNode.equation())
	
	emailsNode.equation("NumberEmailsPerDay=Uniform(0,35)")
	print("Uniform mean, SD:", emailsNode.mean(), emailsNode.sd())
	
	emailsNode.equation("NumberEmailsPerDay=Poisson(3)")
	print("[Poisson lambda=3]")
	print("Poisson mean, SD:", emailsNode.mean(), emailsNode.sd())
	
	emailsNode.equation("NumberEmailsPerDay=Poisson(Subscriptions/2)")
